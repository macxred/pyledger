...
SAMPLE_CSV = """
    __path__,              date,      account,  contra, currency,  amount
...
def test_save_files(tmp_path):
    ...
    expected = SAMPLE_DF.query("__path__ == 'file1.csv'")
    output = pd.read_csv(tmp_path / "file1.csv", skipinitialspace=True)
    assert_frame_equal(
        output, expected, ignore_row_order=True, ignore_columns=["__path__"]
    )

    expected = SAMPLE_DF.query("__path__ == 'level1/file2.csv'")
    output = pd.read_csv(tmp_path / "level1/file2.csv", skipinitialspace=True)
    assert_frame_equal(
        output, expected, ignore_row_order=True, ignore_columns=["__path__"]
    )

    expected = SAMPLE_DF.query("__path__ == 'level1/level2/file3.csv'")
...
def test_save_files_remove_empty_files(tmp_path):
    ...
    save_files(SAMPLE_DF.query("__path__ == 'file1.csv'"), tmp_path)
...
def test_save_files_empty_dataframe_with_path_column(tmp_path):
    empty_df = pd.DataFrame({"__path__": []})
...
def test_save_files_overwrite(tmp_path):
    df = SAMPLE_DF.query("__path__ == 'file1.csv'").copy()
